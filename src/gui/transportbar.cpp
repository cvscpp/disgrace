...
: Fl_Group(X, Y, W, H), m_engine(engine)
{
    ...
    m_metronome = new Fl_Light_Button(x, Y+5, btn_w, btn_h, "Metronome");
    m_metronome->value(1);
    m_metronome->callback(cb_metronome, this);
    x += btn_w + btn_spacing;

    int counter_w = btn_w / 2;
    int label_w = 45;
    ...
    x += label_w + counter_w + btn_spacing;
    ...
    Fl_Simple_Counter* octave_in = new Fl_Simple_Counter(x + label_w, Y+5, counter_w, btn_h, "Octave");
    octave_in->range(0, 9);
    octave_in->step(1);
